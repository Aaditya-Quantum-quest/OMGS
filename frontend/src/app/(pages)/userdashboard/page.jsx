...
//   useEffect(() => {
//     fetchUserData();
//     fetchNameplateOrders(); // Call separately
//   }, []);
...
//       // Separate function to fetch nameplate orders
//       const fetchNameplateOrders = async () => {
//         setLoadingNameplates(true);
//         try {
//           const token = localStorage.getItem('token');
//           if (!token) {
//             console.log('⚠️ No token found for nameplate orders');
//             setNameplateOrders([]);
//             return;
//           }
...
//           console.log('🔍 Fetching nameplate orders...');

//           const response = await axios.get(
//             `${API_URL}/user/orders/nameplates/user`,
//             config
//           );

//           console.log('✅ Nameplate orders fetched:', response.data?.length || 0);
//           console.log('📦 Nameplate data:', response.data);

//           setNameplateOrders(response.data || []);
//         } catch (error) {
//           console.error('❌ Nameplate orders fetch error:', error.response?.data || error.message);
//           setNameplateOrders([]);
//         } finally {
...
// import React, { useState, useEffect } from 'react';
// import { User, Package, MapPin, Loader2, Calendar, CreditCard, ShoppingBag, Image as ImageIcon, Phone, Mail, Palette } from 'lucide-react';
// import axios from 'axios';
// import { useParams } from 'next/navigation';
...
//               </div>
//             </div>
//             <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl sm:rounded-2xl">
//               <Package className="w-6 h-6 sm:w-8 sm:h-8 text-purple-600 flex-shrink-0" />
//               <div className="min-w-0 flex-1">
//                 <p className="text-xs sm:text-sm font-medium text-gray-500">Frame Orders</p>
//                 <p className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
//                   {orders.length}
//                 </p>
//               </div>
//             </div>
//             <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl sm:rounded-2xl">
//               <Palette className="w-6 h-6 sm:w-8 sm:h-8 text-orange-600 flex-shrink-0" />
//               <div className="min-w-0 flex-1">
//                 <p className="text-xs sm:text-sm font-medium text-gray-500">Nameplate Orders</p>
//                 <p className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
...
//               <Package className="w-8 h-8 sm:w-10 sm:h-10 text-indigo-600 bg-indigo-100 p-2 sm:p-3 rounded-xl sm:rounded-2xl flex-shrink-0" />
//               <div className="min-w-0 flex-1">
//                 <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">My Frame Orders</h2>
//                 <p className="text-sm sm:text-base text-gray-600">Track your beautiful frames</p>
//               </div>
//             </div>
...
//           <div className="bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-xl border border-white/50 p-4 sm:p-6 lg:p-8">
//             <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
//               <Palette className="w-8 h-8 sm:w-10 sm:h-10 text-orange-600 bg-orange-100 p-2 sm:p-3 rounded-xl sm:rounded-2xl flex-shrink-0" />
//               <div className="min-w-0 flex-1">
//                 <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">My Nameplate Orders</h2>
//                 <p className="text-sm sm:text-base text-gray-600">Custom nameplate designs</p>
//               </div>
//             </div>
...
//                 <h3 className="text-xl sm:text-2xl font-semibold text-gray-500 mb-2">No nameplate orders yet</h3>
//                 <p className="text-sm sm:text-base text-gray-400 mb-6 sm:mb-8 max-w-md mx-auto px-4">
...
//                   <div key={order._id} className="group border border-gray-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 hover:border-orange-300 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-b from-white to-orange-50/30">
//                     <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 pb-4 sm:pb-6 border-b border-gray-100 gap-3 sm:gap-4">
//                       <div className="min-w-0 flex-1">
//                         <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-1 truncate">Order #{order.orderId}</h3>
//                         <p className="text-sm sm:text-base text-gray-500">{formatDate(order.createdAt)}</p>
...
//                           {order.status?.toUpperCase() || 'PENDING'}
//                         </span>
//                         <span className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold ${getPaymentStatusColor(order.payment?.status)}`}>
...
//                       {/* Design Preview */}
//                       {order.customizedImage && (
//                         <div className="w-full lg:w-64 flex-shrink-0">
//                           <p className="text-sm font-semibold text-gray-700 mb-2">Design Preview</p>
//                           <div className="bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl overflow-hidden border-2 border-gray-200 aspect-[5/4]">
//                             <img 
//                               src={order.customizedImage} 
...
//                             <p><span className="font-medium text-gray-700">Name:</span> {order.customer?.firstName} {order.customer?.lastName}</p>
//                             <p><span className="font-medium text-gray-700">Phone:</span> {order.customer?.phone}</p>
//                             <p className="sm:col-span-2"><span className="font-medium text-gray-700">Email:</span> {order.customer?.email}</p>
//                           </div>
//                         </div>
...
//                           <div className="bg-purple-50 p-3 rounded-lg">
//                             <span className="text-purple-600 font-semibold block mb-1 text-sm">Size</span>
//                             <p className="font-bold text-gray-900">{order.size?.name}</p>
//                             <p className="text-xs text-gray-600">{order.size?.dimensions}</p>
//                           </div>
//                           <div className="bg-emerald-50 p-3 rounded-lg">
//                             <span className="text-emerald-600 font-semibold block mb-1 text-sm">Price</span>
//                             <p className="font-bold text-gray-900 text-lg">₹{order.size?.price}</p>
//                           </div>
//                           <div className="bg-orange-50 p-3 rounded-lg">
//                             <span className="text-orange-600 font-semibold block mb-1 text-sm">Payment</span>
//                             <p className="font-bold text-gray-900 uppercase text-sm">{order.payment?.method}</p>
//                           </div>
//                         </div>
...
const UserDashboard = () => {
  ...
  //     // ✅ FIXED URL - Match your backend mounting
  //     const response = await axios.get(
  //       `${API_URL}/nameplateorders`, // Remove /user/orders/... prefix
  //       config
  //     );
  ...
  const fetchNameplateOrders = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_URL}/nameplateorders`, config);
      ...
      // ✅ ENSURE ARRAY - Fix the main issue!
      const orders = Array.isArray(response.data) ? response.data : [];

      console.log('✅ Nameplate orders processed:', orders.length);
      setNameplateOrders(orders);
    } catch (error) {
      console.error('❌ Nameplate orders fetch error:', error.response?.data || error.message);
      setNameplateOrders([]); // Always set empty array on error
    } finally {
      setLoadingNameplates(false);
    }
  };
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.patch(
        `${API_URL}/user/orders/${orderId}/cancel`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      // Refresh orders
  ...
  const handleContactSupport = (order) => {
    const subject = `Order #${order.orderId} - ${order.status}`;
    const body = `Hi Team,\n\nI have an issue with order #${order.orderId}:\n\nStatus: ${order.status}\nDate: ${formatDate(order.createdAt)}\n\nPlease help.\n\nThanks`;
                  ...
                  <div key={order._id} className="group border border-gray-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 hover:border-indigo-300 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-b from-white to-gray-50">
                    ...
                    <div className="flex flex-wrap gap-3 pt-6 mt-6 border-t border-gray-100">
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition-all duration-200"
                      >
                        Track Order
                      </button>
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-xl hover:bg-emerald-700 transition-all duration-200"
                      >
                        Reorder
                      </button>
                      <button
          ...
          {addresses.length > 0 && (
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-xl border border-white/50 p-4 sm:p-6 lg:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {addresses.map((address) => (
                  <div key={address._id} className="p-6 border border-gray-200 rounded-xl hover:shadow-lg transition-all duration-200">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h4 className="font-bold text-lg text-gray-900">{address.name}</h4>
